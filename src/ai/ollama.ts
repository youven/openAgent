...
export async function* streamAI(messages: ChatMessage[], model: string, token: string) {
  try {
...
export async function callAI(messages: ChatMessage[], model: string, token: string): Promise<string> {
  try {
    const res = await fetch('https://ollama.com/api/chat', {
      method: 'POST',
      headers: {
    ...
    return marked.parse(markdownReply) as string;
